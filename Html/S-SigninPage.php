<?php
	...
	if (isset($_POST['submit'])) {
		...
		$emailCheck = mysqli_query($mysqli,"SELECT * FROM `user_table` WHERE ( `email` = '$email') LIMIT 1");
		$emailErr = false;
		$passwordErr = false;
		if (mysqli_num_rows($emailCheck)!=0) {
			$res = mysqli_fetch_array($emailCheck);
			if ($res['e_password'] == $password) {
...
<body class="m-0 p-0">
            ...
            <div class="col-xs-12 col-md-8 col-lg-8 row justify-content-center collapse navbar-collapse" id="collapsibleNavId">
                <ul class="navbar-nav row justify-content-center text-center">
                    <li class="nav-item mr-lg-2 order-lg-last">
                        <a class="nav-link" href="S-SignupPage.php" tabindex="-1" aria-disabled="true">
                            <button class="btn btn-outline-secondary btn-sm px-3" type="button">Sign Up</button>
                        </a>
					</li>
	...
	<div id="firstSection" class="row container-fluid text-dark mt-5 py-5 justify-content-center">
		<div class="row col-10 m-0 p-0 mt-5 justify-content-center rounded border border-secondary">
			<div class="col-6 m-0 p-0 d-none d-md-block align-self-center">
				<img src="..\to be use\lampcover1.png" class="img-fluid m-0 p-0" width="100%"  alt="pic">
			</div>
			<div class="row col-12 col-md-6 m-0 p-0 bg-gray h-60">
				<form action="" method="POST" class="m-0 p-0 h-100">
					<div class="row col-12 justify-content-center pt-3 m-0 h-100">
						<h2 class="text-white col-12 text-center text-dark">SIGN IN</h2>
						<label class="col-12 my-0 py-0 font-weight-bold">Username/email:</label>
						<div class="form-group col-12 mt-2">
							<input name="e_mail" type="email" class="form-control no-outline input1 bg-gray" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Username or email">
							<label id="emailErr" class="col-12 m-0 p-0 mt-1 text-danger d-none">*email does not exist.</label>
						</div>
						<label class="col-12 my-0 py-0 font-weight-bold">Password:</label>
						<div class="form-group col-12 mt-2">
							<input name="password" type="password" class="form-control no-outline input1 bg-gray" id="exampleInputPassword1" placeholder="Password">
							<label id="passwordErr" class="col-12 m-0 p-0 mt-1 text-danger d-none">*password is  not correct.</label>
						</div>
						<div class="row col-12 justify-content-center my-3">
							<input type="submit" name="submit" value="Sign In" class="col-6  text-secondary btn btn-outline-secondary bg-scheme2 btn-sm px-3 mr-3">
							<div class="col-12 text-center mt-3">
								New to ideas?
								<a id="signInLink" href="S-SignupPage.php">Sign Up</a>
							</div>
						</div>
					</div>
				</form>
